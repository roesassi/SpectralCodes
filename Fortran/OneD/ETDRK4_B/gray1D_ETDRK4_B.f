...
c Constant time step: 4th order ETDRK4-B (Krogstad)
...
c     useful variables
      ...
      open(50,file='gray1D_ETDRK4_B.dat',status='unknown')
...
c +++++++++++++++++++initial condition++++++++++++++++++++++++++
         ...
         u(1,k)=1d0-0.5d0*(dsin(pi*(yf(k)-length)/(2d0*length)))**100d0
         u(2,k)=0.25d0*(dsin(pi*(yf(k)-length)/(2d0*length)))**100d0
         enddo
