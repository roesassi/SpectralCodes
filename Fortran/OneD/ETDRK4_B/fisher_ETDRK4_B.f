c Solves Fisher's equation 1D problem from the text
c Constant time step: 4th order ETDRK4-B (cf Krogstad)
...
* ETDRK4-B variables
     ...
     + m),elr2(n,m),zero,lr2(n,m),four,three,two
      double precision q(n),qb1(n),qb2(n),qc1(n)
     +,qc3(n),f1(n),f2(n),f3(n)
* COMMON BLOCKS
...
c     useful variables
      ...
      open(50,file='fisher_ETDRK4_B.dat',status='unknown')
...
c +++++++++++++++++++ set up etdrk4-b +++++++++++++++++++++++++
c cf Kassam \& Treffethen
      do k=1,n
            u1(1,k)=u(1,k)
            u1(2,k)=u(2,k)
         ee2(k)=dexp(-wavey2(k)*dt/2d0)
         ee(k)=dexp(-wavey2(k)*dt)
       enddo
       ...
       enddo
          ...
          do k=1,n
          do j=1,m
           lr(k,j)=-dcmplx(dt*wavey2(k),0d0)+roots(j)
       ...
       do k=1,n
          do j=1,m
             q(k)=q(k)+dble((elr2(k,j)-one)/lr(k,j))
             qb1(k)=qb1(k)+dble((elr2(k,j)*(lr(k,j)-four)
     +       +lr(k,j)+four)/lr2(k,j))
             qb2(k)=qb2(k)+dble((four*elr2(k,j)-
     +         two*lr(k,j)-four)/lr2(k,j))
             qc1(k)=qc1(k)+dble(((lr(k,j)-two)*
     +       elr(k,j)+lr(k,j)+two)/lr2(k,j))
             qc3(k)=qc3(k)+dble((two*elr(k,j)
     +        -two*lr(k,j)-two)/lr2(k,j))

             f1(k)=f1(k)+dble((elr(k,j)*
     +        (four-three*lr(k,j)+lr2(k,j))-
     +        four-lr(k,j))/lr3(k,j))

             f2(k)=f2(k)+dble((elr(k,j)*(-two+lr(k,j))+
     +        two+lr(k,j))/lr3(k,j))

             f3(k)=f3(k)+dble((elr(k,j)*(four-lr(k,j))-
     +        four-three*lr(k,j)-lr2(k,j))/lr3(k,j))

          enddo
       q(k)=dt*q(k)/dble(m)
       qb1(k)=dt*qb1(k)/dble(m)
       qb2(k)=dt*qb2(k)/dble(m)
       qc1(k)=dt*qc1(k)/dble(m)
       qc3(k)=dt*qc3(k)/dble(m)
       f1(k)=dt*f1(k)/dble(m)
       f2(k)=dt*f2(k)/dble(m)
       f3(k)=dt*f3(k)/dble(m)
       enddo
...
c     +++++++++++++++++++ a loop taken until print out ++++++++++
       ...
       do k=1,n
          call righthandside(a(1,k),u1(1,k))
          a(2,k)=0d0
       enddo
       ...
       do k=1,n
      fftu2(1,k)=ee2(k)*fftu(1,k)+q(k)*ffta(1,k)
          fftu2(2,k)=0d0
       enddo
       call fft1(fftu2,u2,n,nfay,ifay,+1,trigy)

       do k=1,n
          call righthandside(b(1,k),u2(1,k))
          b(2,k)=0d0
       enddo
       call fft1(b,fftb,n,nfay,ifay,-1,trigy)

       do k=1,n
      fftu3(1,k)=ee2(k)*fftu(1,k)+
     +qb1(k)*ffta(1,k)+qb2(k)*fftb(1,k)
          fftu3(2,k)=0d0
       enddo
       call fft1(fftu3,u3,n,nfay,ifay,+1,trigy)

       do k=1,n
          call righthandside(c(1,k),u3(1,k))
          c(2,k)=0d0
       enddo
       call fft1(c,fftc,n,nfay,ifay,-1,trigy)

       do k=1,n
       fftu4(1,k)=ee(k)*fftu(1,k)+
     + qc1(k)*ffta(1,k)+qc3(k)*fftc(1,k)
          fftu4(2,k)=0d0
       enddo
       call fft1(fftu4,u4,n,nfay,ifay,+1,trigy)

          do k=1,n
          call righthandside(d(1,k),u4(1,k))
          d(2,k)=0d0
          enddo
       call fft1(d,fftd,n,nfay,ifay,-1,trigy)

        do k=1,n
       fftu(1,k)=ee(k)*fftu(1,k)+f1(k)*ffta(1,k)
     . +2d0*f2(k)*(fftb(1,k)+fftc(1,k))+f3(k)*fftd(1,k)
          fftu(2,k)=0d0
          enddo
...
c     +++++++++++++++++ end of time loop  ++++++++++++++++++++++++
      ...
      write(*,*) 'Time= ',time
      do k=1,n
        write(50,*) sngl(yf(k)),sngl(u1(1,k))
      enddo
...
c     +++++++++++++++++ the nonlinear terms in the pde +++++++++++++
      subroutine righthandside(rhs1,u)
      implicit none
      double precision rhs1,u
* COMMON BLOCKS
      double precision lambda,epsilon
      common /parms/ lambda,epsilon
      rhs1=u*(1d0-u)
