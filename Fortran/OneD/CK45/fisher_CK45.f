...
c     stores the data from 5th order scheme
      open(51,file='fisher_CK45.dat',status='unknown')
c     stores the data from 4th order scheme
      open(52,file='fishstar_CK45.dat',status='unknown')
...
c     +++++++++++++++++++ a loop taken until print out ++++++++++
 ...
 5    do k=1,n
         ...
         ee2(k)=dexp(-wavey2(k)*dt)
           call righthandside(a(1,k),u1(1,k),dt)
          a(2,k)=0d0 
      enddo
     ...
     +*ffta(1,k)
       ...
       do k=1,n
          call righthandside(b(1,k),u2(1,k),dt)
          b(2,k)=0d0
       enddo
     ...
     +*fftb(1,k)          
       ...
       do k=1,n
          call righthandside(c(1,k),u3(1,k),dt)
          c(2,k)=0d0
       enddo
     ...
     +*fftc(1,k)
       ...
       call fft1(fftu4,u4,n,nfay,ifay,+1,trigy)
          ...
          do k=1,n
          call righthandside(d(1,k),u4(1,k),dt)
          d(2,k)=0d0
          enddo
     ...
     +*fftd(1,k)
       ...
       call fft1(fftu5,u5,n,nfay,ifay,+1,trigy)
          ...
          do k=1,n
          call righthandside(e(1,k),u5(1,k),dt)
          e(2,k)=0d0
          enddo
     ...
     ++brk(6,5)*dexp(-(wavey2(k))*dt*(ark(6)-ark(5)))*ffte(1,k)
      ...
      enddo
       ...
       call fft1(fftu6,u6,n,nfay,ifay,+1,trigy)
          ...
          do k=1,n
          call righthandside(f(1,k),u6(1,k),dt)
          f(2,k)=0d0
          enddo
...
c failed time steps
          if(dabs(uerror).gt.etol) then
          dtnew=0.95d0*dt*(dabs(etol/uerror))**0.25d0
          ifail=ifail+1
          write(41,*) time,dt,dtnew,uerror,ifail
...
c successful time step
       ...
       time=time+dt
       dtnew=0.95d0*dt*(dabs(etol/uerror))**0.2d0
       write(40,*) time,dt,uerror,dtnew
...
c     printing out data
         do k=1,n
               write(51,*) sngl(yf(k)),u1(1,k),ustar(1,k)
               write(52,*) ustar(1,k)
         enddo
...
c     +++++++++++++++++ the nonlinear terms in the pde +++++++++++++
      subroutine righthandside(rhs1,u,dt)
      implicit none
      double precision rhs1,u,dt
* COMMON BLOCKS
      double precision lambda,epsilon
      common /parms/ lambda,epsilon
      rhs1=dt*u*(1d0-u)
