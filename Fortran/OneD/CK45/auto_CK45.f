...
c     stores the data from 5th order scheme
      open(51,file='auto_CK45.dat',status='unknown')
c     stores the data from 4th order scheme
      open(52,file='autostar_CK45.dat',status='unknown')
...
c +++++++++++++++++++initial condition++++++++++++++++++++++++++
         do k=1,n
          ...
          u1(1,k)=0.5d0*(1d0+dtanh((10d0-dabs(yf(k)))/0.1d0))
          u1(2,k)=1d0-0.25d0*(1d0+dtanh((10d0-dabs(yf(k)))/0.1d0))
         enddo
...
c enforce symmetry
 ...
 5    do k=1,n
     ...
     + *ark(2)*delta)+brk(2,1)*dexp(-(wavey2(k))*dt*
     +delta*(ark(2)))*ffta(2,k)
       enddo
     ...
     + *ark(3)*delta)+brk(3,1)*dexp(-(wavey2(k))*dt*
     +delta*ark(3))*ffta(2,k)+brk(3,2)*dexp(-(wavey2(k))*dt*
     +delta*(ark(3)-ark(2)))*fftb(2,k)
       enddo
     ...
     + *ark(4)*delta)+brk(4,1)*dexp(-(wavey2(k))*dt*
     +delta*(ark(4)))*ffta(2,k)
     + +brk(4,2)*dexp(-(wavey2(k))*dt*
     +delta*(ark(4)-ark(2)))*fftb(2,k)
     + +brk(4,3)*dexp(-(wavey2(k))*dt*
     +delta*(ark(4)-ark(3)))*fftc(2,k)
       enddo
     ...
     + *ark(5)*delta)+brk(5,1)*dexp(-(wavey2(k))*dt*
     +delta*ark(5))*ffta(2,k)
     + +brk(5,2)*dexp(-(wavey2(k))*dt*
     +delta*(ark(5)-ark(2)))*fftb(2,k)
     + +brk(5,3)*dexp(-(wavey2(k))*dt*
     +delta*(ark(5)-ark(3)))*fftc(2,k)
     + +brk(5,4)*dexp(-(wavey2(k))*dt*
     +delta*(ark(5)-ark(4)))*fftd(2,k)
          enddo
     ...
     + *ark(6)*delta)+brk(6,1)*dexp(-(wavey2(k))*dt*
     +delta*ark(6))*ffta(2,k)
     + +brk(6,2)*dexp(-(wavey2(k))*dt*
     +delta*(ark(6)-ark(2)))*fftb(2,k)
     + +brk(6,3)*dexp(-(wavey2(k))*dt*
     +delta*(ark(6)-ark(3)))*fftc(2,k)
     + +brk(6,4)*dexp(-(wavey2(k))*dt*
     +delta*(ark(6)-ark(4)))*fftd(2,k)
     + +brk(6,5)*dexp(-(wavey2(k))*dt*
     +delta*(ark(6)-ark(5)))*ffte(2,k)
      enddo
     ...
     + crkstar(1)*ffta(2,k)
     ++crkstar(3)*fftc(2,k)*
     +dexp(delta*(wavey2(k))*dt*ark(3))
     ++crkstar(4)*fftd(2,k)*
     +dexp(delta*(wavey2(k))*dt*ark(4))
     ++crkstar(5)*ffte(2,k)*
     +dexp(delta*(wavey2(k))*dt*ark(5))
     ++crkstar(6)*fftf(2,k)*
     +dexp(delta*(wavey2(k))*dt*ark(6)))
