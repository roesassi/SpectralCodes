...
c      common /parms/ a0, a1, epsilon, delta, vminus
...
c     useful variables
      ...
      open(51,file='gray_ETDRK4_B.dat',status='unknown')
...
c     +++++++++++++++++++ initial condition ++++++++++++++++++++++++++
      do i=1,lenh
         ...
         u(i,1,k)=1d0-0.5d0*dexp(-0.05d0*radius2)
         u(i,2,k)=0.25d0*dexp(-0.05d0*radius2)
         enddo
      enddo
...
c     +++++++++++++++++++ a loop taken until print out ++++++++++
         do count=1,ncount 
            ...
            do k=1,n
               do ij=1,lenh
                  call righthandside(a(ij,1,k),a(ij,2,k)
     +            ,u1(ij,1,k),u1(ij,2,k),dt)
               enddo
            enddo
     ...
     +fftffta(ij,2,k)
            ...
            do k=1,n
               do ij=1,lenh
                  call righthandside(b(ij,1,k),b(ij,2,k)
     +            ,u2(ij,1,k),u2(ij,2,k),dt)
               enddo
            enddo
     ...
     +qb1(ij,k,2)*fftffta(ij,2,k)+qb2(ij,k,2)*fftfftb(ij,2,k)
            ...
            do k=1,n
               do ij=1,lenh
                  call righthandside(c(ij,1,k),c(ij,2,k)
     +            ,u3(ij,1,k),u3(ij,2,k),dt)
               enddo
            enddo
     ...
     + qc1(ij,k,2)*fftffta(ij,2,k)+qc3(ij,k,2)*fftfftc(ij,2,k)
            ...
            do k=1,n
               do ij=1,lenh
                  call righthandside(d(ij,1,k),d(ij,2,k)
     +            ,u4(ij,1,k),u4(ij,2,k),dt)
               enddo
            enddo
