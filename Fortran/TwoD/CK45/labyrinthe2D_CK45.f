...
c     stores the data from 5th order scheme
      open(51,file='laby_CK45.dat',status='unknown')
c     stores the data from 4th order scheme
      open(52,file='labystar_CK45.dat',status='unknown')
...
c     +++++++++++++++++++ initial condition ++++++++++++++++++++++++++
      do i=1,lenh
         ...
         do k=1,n
            ...
            u1(i,1,k)=a1*vminus+a0-
     +           2d0*a1*((2d0*vminus*dexp(-0.1*radius**2)))
            u1(i,2,k)=vminus-(2d0*vminus*dexp(-0.1d0*radius**2))
         enddo
      enddo
...
c successful time step
      ...
      if(dabs(time-tend).lt.1d-14) then
         time=tend
