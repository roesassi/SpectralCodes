function labyrinthe2D_ETDRK4(N,Nfinal,dt,ckeep,L,a0,a1,varepsilon,delta)
if nargin<9
...
B21u=rsym(B21u); C1u=rsym(C1u); C23u=rsym(C23u); C4u=rsym(C4u);
B21v=rsym(B21v); C1v=rsym(C1v); C23v=rsym(C23v); C4v=rsym(C4v);
disp('Coefficients preparation completed')
%-----------------ETD Runge-Kutta----------------------------------
for n=1:Nfinal,
    [dui,dvi]=rhside(u,v,rhside_par{:});
    k1u=dt*fft2(dui); k1v=dt*fft2(dvi);
    uhat2=Eu2.*uhat + B21u.*k1u;
    vhat2=Ev2.*vhat + B21v.*k1v;
    u2=real(ifft2(uhat2)); v2=real(ifft2(vhat2));
    [dui,dvi]=rhside(u2,v2,rhside_par{:});
    k2u=dt*fft2(dui); k2v=dt*fft2(dvi);
    u3=real(ifft2(Eu2.*uhat + B21u.*k2u));
    v3=real(ifft2(Ev2.*vhat + B21v.*k2v));
    [dui,dvi]=rhside(u3,v3,rhside_par{:});
    k3u=dt*fft2(dui); k3v=dt*fft2(dvi);
    u4=real(ifft2(Eu2.*uhat2 + B21u.*(2*k3u-k1u)));
    v4=real(ifft2(Ev2.*vhat2 + B21v.*(2*k3v-k1v)));
...
save('labyrinthe2D_ETDRK4.mat','tkeep','ukeep','vkeep','N','L','x','y','Nfinal','ckeep')
%--------------Initial Condition ---------------------------------
